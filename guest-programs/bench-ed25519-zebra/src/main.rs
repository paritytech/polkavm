...
// `ed25519-zebra` is what `sp_core::ed25519` verifies with (ZIP-215 rules);
// the sibling `bench-ed25519` uses `ed25519-dalek` (strict RFC 8032).
// Both use the same RFC 8032 test vector below, so the two implementations
...
fn benchmark_run(_state: &mut State) {
    ...
    let key = ed25519_zebra::VerificationKey::try_from(black_box(PUBLIC_KEY)).unwrap();
    let signature = ed25519_zebra::Signature::from(black_box(SIGNATURE));
    black_box(key.verify(&signature, &MESSAGE)).unwrap();
}
