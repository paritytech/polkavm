...
fn test_advance_pc_and_const_add_pc_debug_info_64() {
    ...
    //// NOTE: this file is generated by resolc base on LLVM 21
    //let elf = decompress_zstd(include_bytes!("../../../test-data/revive-transfer-example.zst"));

    //// NOTE: this file is generated by resolc base on LLVM 18 (current main)
    ...
    for pc in 0..=code_length + 1 {
        let pc = ProgramCounter(pc);
        let mut line_program = program.get_debug_line_program_at(pc);
        if !line_program.is_ok() || line_program.as_ref().unwrap().is_none() {
            continue;
        }
        println!("entry point: {pc}");
    }
