...
use utils::constants::{NONE, PAGE_SIZE, SEGMENT_SIZE, FIRST_READABLE_ADDRESS, FIRST_READABLE_PAGE};
use utils::functions::{parse_refine_args, parse_accumulate_args};
use utils::functions::{setup_page, get_page, serialize_gas_and_registers, deserialize_gas_and_registers, initialize_pvm_registers};
use utils::functions::{write_result};

use utils::host_functions::{historical_lookup, fetch, export, machine, peek, invoke, expunge};
use utils::host_functions::{gas, lookup, read, write, info, bless, assign, checkpoint, new, upgrade, eject, query, solicit, forget, oyield};
...
extern "C" fn accumulate(start_address: u64, length: u64) -> (u64, u64) {
    ...
    if n == 1 {
        ...
        let read_none_result = unsafe { read(service_index as u64, jam_key_address, jam_key_length, buffer_address, 0, buffer_length) };
        ...
        let read_ok_result = unsafe { read(service_index as u64, jam_key_address, jam_key_length, buffer_address, 0, buffer_length) };
        ...
        let read_result = unsafe { read(service_index as u64, jam_key_address, jam_key_length, buffer_address, 0, buffer_length) };
