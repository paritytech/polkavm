...
use utils::functions::{z_func};

use utils::host_functions::{solicit};
use utils::host_functions::{export, expunge, fetch, historical_lookup, invoke, machine, poke, peek, zero, log, manifest};
...
extern "C" fn refine(start_address: u64, length: u64) -> (u64, u64) {
    ...
    child_vm_registers[7] = 2 * Z_Z + z_func(o_bytes_length) as u64;
    ...
    // reset manifest
    const manifest_max_length: usize = 3072 * 4;
    let mut manifest_buf = [0u8; manifest_max_length];
    let manifest_buf_address = manifest_buf.as_mut_ptr() as u64;
    let manifest_result = unsafe { manifest(new_machine_idx as u64, 0, manifest_buf_address, manifest_max_length as u64, 0) };
    call_log(2, None, &format!("Parent: manifest reset result={:?}", manifest_result));

    manifest_buf.fill(0);
    let manifest_result = unsafe { manifest(new_machine_idx as u64, 1, manifest_buf_address, manifest_max_length as u64, 0) };
    call_log(2, None, &format!("Parent: manifest length (before invoke): ={:?}", manifest_result));
    ...
    manifest_buf.fill(0);
    let manifest_result = unsafe { manifest(new_machine_idx as u64, 1, manifest_buf_address, manifest_max_length as u64, 0) };
    call_log(2, None, &format!("Parent: manifest length (after invoke): ={:?}", manifest_result));

    // peek child VM output and export it
    let output_start_address = child_vm_registers[7];
    let output_length = child_vm_registers[8];
    let num_of_output_pages = output_length / PAGE_SIZE as u64;

    for i in 0..num_of_output_pages {
        segment_buf.fill(0);
        ...
        let manifest_result = unsafe { manifest(new_machine_idx as u64, 3, manifest_buf_address, manifest_max_length as u64, page_id) };
        if manifest_result == OK {
            call_log(2, None, &format!("Parent: page_id={:?} is all zero" , page_id));
        } else {
            unsafe { export(segment_buf_segment_address, SEGMENT_SIZE) };
        }
        call_log(2, None, &format!("Parent: segment_buf[0..8]={:?}", &segment_buf[0..8]));
    }
