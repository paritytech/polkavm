...
#[no_mangle]
static mut output_pages: [u8; TOTAL_BYTES] = [0; TOTAL_BYTES];
...
extern "C" fn main(start_address: u64, length: u64, step_n: u64, num_of_gloders: u64, total_execution_steps: u64) -> (u64, u64) {
    ...
    if length > 0 {
        for i in 0..NUM_PAGES {
            // grid.pages[i] = unsafe {
            //     core::ptr::read_volatile((start_address + ((i as u64) * (PAGE_SIZE as u64))) as *const [u8; PAGE_SIZE])
            // };
            let ptr = (start_address + ((i as u64) * (PAGE_SIZE as u64))) as *const u8;
            let slice = unsafe { core::slice::from_raw_parts(ptr, PAGE_SIZE) };
            grid.pages[i].copy_from_slice(slice);
        }
        call_log(2, None, &format!("Child: Init from input address: {:?}", start_address));
    } else {
        grid.init_gliders(num_of_gloders as usize);
        call_log(2, None, &format!("Child: Init gliders: {:?}", num_of_gloders));
    }

    for _ in 0..total_execution_steps {
        grid = grid.step();
    }
    call_log(2, None, &format!("Child: step_n={:?} total_execution_steps={:?}", step_n, total_execution_steps));

    // write all pages of grid to output_pages
    for i in 0..NUM_PAGES {
        unsafe {
            output_pages[i * PAGE_SIZE..(i + 1) * PAGE_SIZE].copy_from_slice(&grid.pages[i]);
        }
    }

    // get the address of output_pages
    let output_address = unsafe { output_pages.as_ptr() as u64 };
    let output_length = (PAGE_SIZE * NUM_PAGES) as u64;
    call_log(2, None, &format!("Child: output address={:?} length={:?}", output_address, output_length));
