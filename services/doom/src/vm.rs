...
impl Vm {
    pub fn from_blob(blob: ProgramBlob) -> Result<Self, polkavm::Error> {
        ...
        linker.define_typed(
            "ext_output_video",
            |caller: Caller<State>, address: u32, width: u32, height: u32| -> u32 {
                let length = width * height * 4;
                caller.user_data.frame.clear();
                caller.user_data.frame.reserve(length as usize);
                caller
                    .instance
                    .read_memory_into(address, &mut caller.user_data.frame.spare_capacity_mut()[..length as usize]).unwrap();
                unsafe {
        ...
        linker.define_typed(
            "ext_output_audio",
            |caller: Caller<State>, address: u32, samples: u32| -> u32 {
...

                {
                    ...
                    caller.instance.read_memory_into(address, audio_buffer).unwrap();
                }
        ...
        linker.define_typed(
            "ext_rom_read",
            |caller: Caller<State>, pointer: u32, offset: u32, length: u32| -> u32 {
                let chunk = caller
                    .user_data
                    .rom
                    .get(offset as usize..offset as usize + length as usize)
                    .ok_or_else(|| format!("invalid ROM read: offset = 0x{offset:x}, length = {length}")).unwrap();

                // Ok(caller.instance.write_memory(pointer, chunk)?)
                caller.instance.write_memory(pointer, chunk).unwrap();
                return 0;
            },
        )?;
        ...
        linker.define_typed(
            "ext_stdout",
            |caller: Caller<State>, buffer: u32, length: u32| -> u32 {
                if length == 0 {
                    // return Ok(0);
                    return 0;
                }
    ...
    pub fn initialize(&mut self, rom: impl Into<Vec<u8>>) -> Result<(), CallError> {
        self.state.rom = rom.into();
        self.instance.call_typed(&mut self.state, "ext_initialize", ()).unwrap();
        Ok(())
    }

    pub fn run_for_a_frame(&mut self) -> Result<(u32, u32, &[u8]), CallError> {
        self.instance.call_typed(&mut self.state, "ext_tick", ()).unwrap();
